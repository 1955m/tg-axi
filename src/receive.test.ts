...
describe("message type normalization", () => {
  ...
  it("normalizes a location", async () => {
    const r = await normalizeOne(
      msg({ message_id: 2, location: { longitude: 12.34, latitude: 56.78 } }),
    );
  ...
  it("normalizes a contact", async () => {
    const r = await normalizeOne(
      msg({
        message_id: 3,
        contact: { phone_number: "+15551234", first_name: "Bob", user_id: 42 },
      }),
    );
  ...
  it("normalizes voice (opus/.oga) with duration + mime and downloads the file", async () => {
    mockFetch((_url) => {
      if (_url.includes("/getFile"))
        return okResult({ file_id: "V1", file_path: "voice/file_1.oga" });
    ...
    const r = await normalizeUpdate(
      {
        ...
        message: msg({
          message_id: 5,
          voice: { file_id: "V1", duration: 9, mime_type: "audio/ogg", file_size: 1234 },
        }),
      },
      CTX,
      opts(),
  ...
  it("normalizes a document of ANY mime (application/pdf) and downloads it", async () => {
    mockFetch((_url) => {
      if (_url.includes("/getFile"))
        return okResult({ file_id: "D1", file_path: "documents/file_1.pdf" });
  ...
  it("normalizes audio, video, video_note, animation, sticker", async () => {
    ...
    const audio = await normalizeUpdate(
      {
        ...
        message: msg({
          message_id: 1,
          audio: { file_id: "A1", duration: 30, mime_type: "audio/mpeg", file_name: "song.mp3" },
        }),
      },
      CTX,
      opts(),
    ...
    const vn = await normalizeUpdate(
      {
        update_id: 3,
        message: msg({ message_id: 3, video_note: { file_id: "VN", duration: 7, length: 240 } }),
      },
      CTX,
      opts(),
    );
    ...
    const anim = await normalizeUpdate(
      {
        ...
        message: msg({
          message_id: 4,
          animation: { file_id: "AN", duration: 2, mime_type: "video/mp4", file_name: "giphy.mp4" },
        }),
      },
      CTX,
      opts(),
...
describe("media download — graceful failure for >20MB / getFile errors", () => {
  ...
  it("emits downloaded=false when the /file/ download HTTP fails", async () => {
    ...
    const r = await normalizeUpdate(
      {
        update_id: 10,
        message: msg({ message_id: 9, document: { file_id: "F", file_name: "f.bin" } }),
      },
      CTX,
      opts(),
    );
  ...
  it("--no-download skips writes but keeps metadata", async () => {
    ...
    mockFetch((_url) => {
      ...
      if (_url.includes("/file/bot")) {
        dlCalls++;
        return { status: 200, bytes: Buffer.from("x") };
      }
      return okResult([]);
    });
    const r = await normalizeUpdate(
      {
        ...
        message: msg({
          message_id: 10,
          voice: { file_id: "F", duration: 1, mime_type: "audio/ogg" },
        }),
      },
      CTX,
  ...
  it("writes the downloaded bytes to the inbox path via writeFile", async () => {
    ...
    mockFetch((_url) => {
      if (_url.includes("/getFile"))
        return okResult({ file_id: "F", file_path: "voice/file_1.oga" });
